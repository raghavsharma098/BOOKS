    ...
    <main className="min-h-screen bg-[#F2F0E4]">
      ...
      <div className="lg:pl-[96px] px-4 sm:px-6 md:px-8 py-6 sm:py-8 md:py-10 w-full max-w-[1280px] mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-6">
          ...
          <div className="bg-[#FFFFFFB2] rounded-[16px] p-5 sm:p-6 shadow-md flex flex-col sm:flex-row gap-4 sm:items-center lg:col-span-2 relative">
            {/* Active Reader badge - absolute on mobile, inline on larger screens */}
            <span className="absolute top-3 right-3 sm:hidden flex items-center justify-center text-[11px] font-sf font-[500] text-white" style={{ width: 'auto', height: '28px', padding: '0 12px', gap: profile.isVerified ? '6px' : '0', borderRadius: '26843500px', background: 'linear-gradient(180deg, #60351B 0%, #4A2816 100%)', boxShadow: '0px 4px 6px -4px #0000001A, 0px 10px 15px -3px #0000001A', opacity: 1 }}>
              {profile.isVerified && <Image src={verifiedIcon} alt="verified" width={14} height={14} className="object-contain" />}
...
<div className="flex-shrink-0 w-[80px] h-[80px] sm:w-[100px] sm:h-[100px] md:w-[128px] md:h-[128px] rounded-[12px] sm:rounded-[14px] md:rounded-[16px] overflow-hidden bg-gray-100 mt-4 sm:-mt-16 md:-mt-[88px]" style={{ paddingLeft: 0, borderTop: '0.8px solid #60351B33', borderWidth: '0.8px', borderStyle: 'solid', boxShadow: '0px 25px 50px -12px #00000040', opacity: 1, transform: 'rotate(0deg)' }}>
              ...
              <div className="flex items-start gap-2 sm:gap-4">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="hidden sm:flex items-center justify-center text-[12px] font-sf font-[500] text-white" style={{ width: '161px', height: '32px', paddingLeft: profile.isVerified ? '16px' : '8px', gap: profile.isVerified ? '8px' : '0', borderRadius: '26843500px', background: 'linear-gradient(180deg, #60351B 0%, #4A2816 100%)', boxShadow: '0px 4px 6px -4px #0000001A, 0px 10px 15px -3px #0000001A', opacity: 1 }}>
          ...
          <div className="bg-[#FFFFFFB2] rounded-[12px] p-3 sm:p-4 md:p-5 shadow-sm lg:col-span-2" style={{ borderTop: '0.8px solid rgba(96, 53, 27, 0.2)' }}>
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 sm:gap-0">
              <div>
                <div className="font-sf font-[590] text-[16px] sm:text-[18px] leading-[22px] sm:leading-[24px] text-[#210C00]">Published Books</div>
                <div className="text-xs sm:text-sm text-[rgba(33,12,0,0.45)] mt-0.5 sm:mt-1">{PUBLISHED_PLACEHOLDERS.length} published books</div>
              </div>
              <a className="text-xs sm:text-sm text-[#8B5A3C] hover:underline">View all works →</a>
            </div>

            <div className="mt-3 sm:mt-4">
              {/* responsive grid: min column 140px on mobile, 188px on larger */}
              <div className="overflow-x-auto sm:overflow-visible -mx-1 px-1">
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 sm:gap-4 md:gap-5">
                  {PUBLISHED_PLACEHOLDERS.map((b, i) => (
                    <div key={i} className="w-full">
                      <div
                        className="w-full h-[180px] sm:h-[220px] md:h-[260px] lg:h-[282px] overflow-hidden"
                        style={{
                      ...
                      <div className="mt-2 sm:mt-3 text-xs sm:text-sm font-[600] text-[#210C00] truncate">{b.title}</div>
                      <div className="text-[10px] sm:text-xs text-[rgba(33,12,0,0.6)] mt-0.5 sm:mt-1">{b.date}</div>

                      <div className="mt-1.5 sm:mt-2 flex items-center gap-1.5 sm:gap-2 text-[10px] sm:text-xs text-[rgba(33,12,0,0.6)]">
                        <svg width="10" height="10" className="sm:w-3 sm:h-3" viewBox="0 0 20 20" fill="#FE9A00" xmlns="http://www.w3.org/2000/svg"><path d="M10 15l-5.878 3.09L5.49 11.454 1 7.91l6.068-.88L10 1l2.932 6.03L19 7.91l-4.49 3.545 1.368 6.636z"/></svg>
                        <div className="font-semibold text-[#210C00]">{b.rating}</div>
                        <div className="text-[10px] sm:text-xs text-[rgba(33,12,0,0.6)]">({b.reviews})</div>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <div>
            ...
            <div className="bg-[#FFFFFFB2] rounded-[12px] sm:rounded-[16px] shadow-sm p-4 sm:p-6 md:p-8 lg:h-[388.59px]" style={{ borderTop: '0.8px solid #60351B33' }}>
              <div className="flex items-start">
                <div>
                  <div className="font-sf font-[590] text-[16px] sm:text-[18px] md:text-[20px] leading-[22px] sm:leading-[26px] md:leading-[28px] text-[#210C00]">Recommended for You</div>
                  <div className="mt-0.5 sm:mt-1 font-sf font-[400] text-[12px] sm:text-[13px] md:text-[14px] leading-[18px] sm:leading-[20px] text-[#210C0099]">Based on your reading preferences and activity</div>
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5 md:gap-6">
                  {recommendedLoading ? (
                    Array.from({ length: RECOMMENDED_PLACEHOLDERS.length }).map((_, idx) => (
                      <div key={idx} className={`flex items-start gap-3 sm:gap-4 ${idx >= 2 && idx < 4 ? 'sm:mt-8' : ''}`}>
                        <div className="w-12 h-16 sm:w-14 sm:h-18 md:w-16 md:h-20 bg-gray-200 flex-shrink-0 animate-pulse" style={{ borderRadius: '6px 2px 2px 6px' }} />
                        <div className="min-w-0 flex-1">
                          <div className="h-3 sm:h-4 bg-gray-200 rounded w-24 sm:w-32 mb-1.5 sm:mb-2 animate-pulse" />
                          <div className="h-2.5 sm:h-3 bg-gray-200 rounded w-20 sm:w-24 mb-1 animate-pulse" />
                          <div className="h-2.5 sm:h-3 bg-gray-200 rounded w-32 sm:w-40 animate-pulse" />
                        </div>
                      </div>
                    ))
                  ) : (
                    recommended.map((it, idx) => (
                      <div key={idx} className={`flex items-start gap-3 sm:gap-4 ${idx >= 2 && idx < 4 ? 'sm:mt-8' : ''}`}>
                        <div className="w-12 h-16 sm:w-14 sm:h-18 md:w-16 md:h-20 overflow-hidden bg-gray-100 flex-shrink-0" style={{ borderRadius: '6px 2px 2px 6px', boxShadow: '7px 0px 4px 0px #00000073 inset, 0px 4px 6px -4px #0000001A, 0px 10px 15px -3px #0000001A, 0px 8px 10px -6px #0000001A, 0px 20px 25px -5px #0000001A, -8px 11px 9px 0px #00000078, 0px 5px 5.3px 0px #FFFFFF40, -24px 27px 22.4px 11px #60351B2B' }}>
                          <img src={it.cover} alt={it.title} className="w-full h-full object-cover" />
                        </div>

                        <div className="min-w-0 flex-1">
                          <div className="font-sf font-[590] text-[13px] sm:text-sm text-[#210C00] truncate">{it.title}</div>
                          <div className="text-[11px] sm:text-xs text-[rgba(33,12,0,0.6)] mt-0.5 sm:mt-1">{it.author}</div>
                          <div className="mt-0.5 sm:mt-1 font-sf font-[400] text-[11px] sm:text-[12px] leading-[16px] sm:leading-[19.5px] text-[#60351B] line-clamp-2">{it.reason}</div>
                        </div>
                      </div>
                    ))
                  )}
            ...
            <section>
              ...
              <div className="mt-4 sm:mt-6">
                ...
                <h2 className="font-sf font-[590] text-[22px] sm:text-[26px] md:text-[30px] leading-[28px] sm:leading-[32px] md:leading-[36px] tracking-[-0.5px] sm:tracking-[-0.65px] md:tracking-[-0.75px] text-[#210C00]">Reviews</h2>
                </div>
